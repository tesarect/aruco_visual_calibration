#!/bin/bash
# WINDOW variant of debug_tmux.sh — see sim_win_base.sh's header for why
# this parallel set of scripts exists (one shared SESSION per environment
# — "sim_deploy" or "real_deploy" — each script adds its own WINDOW
# instead of its own session). Same panes/commands/titles/layout as
# debug_tmux.sh; that script is UNTOUCHED and still the right tool for
# standalone use.
#
# Usage: ./win_debug.sh [sim|real]   (default: sim) — picks both
# tf_debug_markers.py's --env AND which shared session ("sim_deploy" vs
# "real_deploy") this window joins.
...
# Pane 1 — tf_debug_markers.py. Polls for move_group (started by this
# environment's base window — sim_win_base.sh/real_win_base.sh).
...
# Pane 4 — swri_console, a rosout GUI log viewer.
