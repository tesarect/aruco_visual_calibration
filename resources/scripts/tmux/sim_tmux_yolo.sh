...
# yolo_marker_bridge_node. Split out of sim_tmux_trajcal.sh (2026-07-24,
# matching real_tmux_yolo.sh's earlier split) to keep that session down to
# its core calibration-pipeline panes — these two panes are independent of
# it otherwise, only sharing the requirement that move_group (from
# sim_tmux_base.sh) is already up.
#
# Requires the base session (sim_tmux_base.sh) to be up first — pane 1
# polls for move_group. Independent of sim_tmux_trajcal.sh's own panes
...
#   ./sim_tmux_yolo.sh inference_server=on yolo_marker_bridge=on
# or turn logging on for both at once with essential_logs=on:
#   ./sim_tmux_yolo.sh essential_logs=on
...
# for startup-ordering consistency (matches sim_tmux_base.sh being started
# first).
