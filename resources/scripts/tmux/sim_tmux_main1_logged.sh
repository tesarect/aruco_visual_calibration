...
"$SHELL_DIR/wait_for_node.sh move_group 30 && source ~/ros2_ws/install/setup.bash && ros2 run aruco_perception aruco_detector_node --ros-args --params-file ~/ros2_ws/src/visual_calibration/aruco_perception/config/aruco_detector_sim.yaml 2>&1 | tee -a $LOGFILE" C-m
...
"$SHELL_DIR/wait_for_node.sh aruco_detector_node 30 && $SHELL_DIR/wait_for_node.sh trajectory_planner 30 && source ~/ros2_ws/install/setup.bash && ros2 run aruco_perception calibration_broadcaster_node --ros-args --params-file ~/ros2_ws/src/visual_calibration/aruco_perception/config/calibration_broadcaster_sim.yaml 2>&1 | tee -a $LOGFILE" C-m
...
# feedback and the final orientation spread (max/mean degrees). Not teed
# into $LOGFILE: it's an interactive shell, not a long-running node.
PANE3=$(tmux split-window -t "$PANE1" -v -P -F "#{pane_id}")
tmux send-keys -t "$PANE3" \
"$SHELL_DIR/wait_for_node.sh calibration_broadcaster_node 30 && source ~/ros2_ws/install/setup.bash && source $SHELL_DIR/aliases.sh && echo 'Ready. Run: runcalibration' && echo 'Combined log: $LOGFILE'" C-m
