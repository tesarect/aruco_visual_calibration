...
| `bringup_yolo_pipeline.launch.py` | `aruco_perception_yolo_bridge` | `inference_server.py` (via `ExecuteProcess`) → `yolo_marker_bridge_node` | `inference_server.py` is **not a ROS node** (plain Flask process inside `~/yolo_venv`, no `rclpy` import — see `start_inference_server.sh`'s header) so it can't use `launch_ros`'s `Node` action at all; started via `ExecuteProcess` invoking the venv's `python3` directly. Readiness = polling `GET http://127.0.0.1:8601/health` for `"status": "ok"` — replicates `wait_for_inference_server.sh`. Then also waits for `move_group` before starting the bridge node, replicating that pane's full `wait_for_inference_server.sh && wait_for_node.sh move_group` chain. |
